...
class Biosphere3D:
    ...
    def _get_agent_observation(self, agent) -> np.ndarray:
        ...
        in_doubt = agent.get("last_entropy", 0.0)
        ...
        meta_obs = [in_surprise, worm_nearby, in_throw_result, in_doubt]
        v13_obs = [in_slot1_type, in_slot2_type, in_slot1_weight, in_nearby_item_type, in_nearby_item_count]
        
        # Total: 16 (base) + 6 (lidar) + 2 (prey) + 3 (meta) + 5 (v13) = 32 entrées
        full_obs = base_obs + lidar.tolist() + [is_flying, is_stunned] + meta_obs + v13_obs
    ...
    def step(self):
        ...
        for agent in self.agents:
            ...
            preds, H_prev, H_history, H_potentials, surprise = recurrent_forward(
                ...
                env_surprise=agent.get("last_surprise", 0.0)
            )
            ...
            # Calcul du doute (Entropie des actions motrices)
            probs = np.exp(logits[:6]) / (np.sum(np.exp(logits[:6])) + 1e-9)
            entropy = -np.sum(probs * np.log(probs + 1e-9))
            agent["last_entropy"] = min(entropy / 1.79, 1.0) # max entropy is ~1.79 for 6 classes
            ...
            if do_grab:
                for itm in self.items:
                    if itm["x"] == agent["x"] and itm["y"] == agent["y"] and itm["z"] == agent["z"]:
                        ...
                        if len(agent["inventory"]) < inv_capacity:
                            agent["inventory"].append(itm["type"])
                        else:
                            # Drop Forcé (LIFO) et Surprise Cognitive : L'objet ne rentre pas et retombe
                            self.items.append({"x": agent["x"], "y": agent["y"], "z": agent["z"], "type": itm["type"]})
                            agent["last_surprise"] = min(1.0, agent.get("last_surprise", 0.0) + 0.5)
