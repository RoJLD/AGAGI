...
def compare(seeds=(0, 1, 2, 3, 4), train_ep=1200, test_ep=100, n_agents=128):
    ...
    if not list(seeds) or int(train_ep) <= 0 or int(test_ep) <= 0 or int(n_agents) <= 0:
        raise ValueError(
            f"compare : argument degenere (seeds={seeds!r}, train_ep={train_ep}, test_ep={test_ep}, n_agents={n_agents}) -- aucune mesure possible ; "
            "ne pas confondre avec une mesure nulle OBSERVEE.")
    rows = []
    for s in seeds:
