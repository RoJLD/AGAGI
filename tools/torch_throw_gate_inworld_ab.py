...
def compare_factorial(seeds=(0, 1, 2, 3, 4), prey_sparse=15, prey_dense=300, ticks=120, warmup=30,
    ...
    # ⚠️ DEFAUT RELEVE A 5 le 2026-09-15 (P2.60, portage par fusion 3-voies du tag
    # keep/edr-177-178-factorial-regime-sweep) : le tag partait a (0,1,2,3), SOUS le plancher n >= 5 de
    # `compute_ab_verdict` (sign_p = 2 x 0.5^n) -- un bras qui ne peut pas reussir (classe E2), corrige
    # sur les autres `compare_*` de ce fichier le 2026-09-09 (P2.49). Les records EDR-177/178 ont
    # declare K=8 et K=12 a la main : leurs chiffres sont inchanges.
    # GARDE D'ARGUMENTS, EN TETE (E14 retro-appliquee au portage). Un argument degenere est une erreur
    # d'APPEL, pas un fait sur le monde : sans elle, `seeds=()` rendait 16 cellules a `median([])` et un
    # horizon nul un verdict NEUTRE de fond. Posee AVANT toute construction de monde -> refus instantane.
    if not list(seeds) or int(ticks) <= 0 or int(n_agents) <= 0 or int(warmup) >= int(ticks):
        raise ValueError(
            f"compare_factorial : argument degenere (seeds={list(seeds)}, ticks={ticks}, warmup={warmup}, n_agents={n_agents}) -- aucune mesure possible ; "
            "ne pas confondre avec une mesure nulle OBSERVEE.")
    import itertools
    import statistics as _st
...
def _factorial_effects(cells):
    ...
    ⚠️ Porte 14 (2026-09-15, P2.60) : un pool VIDE (niveau jamais mesure) rend `nan`, jamais `0.0` --
    le tag rendait 0.0, c.-a-d. « aucun effet » la ou il n'y avait AUCUNE mesure. Definition UNIQUE :
    tools/factorial_regime_sweep.py (EDR-178) l'importe d'ici."""
    import math
    import statistics as _st
    ...
    def _pool(pred):
        ...
        return _st.mean(vals) if vals else math.nan
