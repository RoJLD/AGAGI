...
with hold("kuzu", owner="evo026-horizon", ttl_s=21600):
    ...
    print("[provenance]", provenance("EVO-026"))   # P2.68 : git_sha / dirty / seal, dans l'evidence (stdout)
    ...
    # COMBIEN de cellules ? TROIS seuils de controle, chacun applique UNE SEULE FOIS. Enumeration,
    # chaque ligne verifiable plus bas :
    #   (a) PRE-VOL volet 2 : `probe_std == 0` -> ARRET (le compteur a-t-il MORDU ? classe E4)
    #   (b) volet 1 in situ : ratio des tirages medians `15 <= dl/ds <= 27`
    #   (c) dv_sante_lignee : `hl >= 0.70 * hs`, lue SEULEMENT dans la branche p >= 0.05
    # (b) et (c) portent sur des MEDIANES deja agregees sur les 12 seeds du bras, jamais sur un seed ;
    # (a) porte sur un compte cumule d'une sonde a graine FIXE. Le Fisher exact est la DV, hors famille.
    ...
    _famille = assert_control_family(
        ...
        reason="Multiplicite SANS OBJET ici : les 3 cellules sont des seuils DETERMINISTES appliques "
               "UNE FOIS chacun, a une mediane deja agregee sur les 12 seeds du bras (ou a un compte "
               "cumule de sonde a graine fixe), jamais a chaque seed. Aucun alpha n'est applique a "
               "une cellule de controle : il n'y a rien a corriger, et une Bonferroni serait "
               ...
               "test a p-value du run est la DV (Fisher exact bilateral), unique.")
