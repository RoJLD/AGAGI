...
def run_seed(config, db, seed, use_head, decode_act, num_agents=24, max_ticks=300, base=0):
    ...
    if int(num_agents) <= 0 or int(max_ticks) <= 0:
        raise ValueError(
            f"run_seed : argument degenere (num_agents={num_agents}, max_ticks={max_ticks}) -- aucune mesure possible ; "
            "ne pas confondre avec une mesure nulle OBSERVEE.")
    seed_at(base, seed)          # APPARIEMENT (D1) : base+seed identique -> MÊME monde pour les 3 conditions
...
def main(seeds=range(6), seed=None):
    with Harness(seed=seed, name="func_benefit", with_db=True) as h:
        db = h.db
        if db is None:
        ...
        seeds = list(seeds)
        base = h.seed
        print(f"BENEFICE FONCTIONNEL du langage fiable : seed={base}, {len(seeds)} seeds, 3 conditions.")
        agg = {name: {"mammoth": [], "leurre": [], "survivors": []} for name, _, _ in CONDITIONS}
        for s in seeds:
            line = f"  seed {s}: "
            for name, uh, da in CONDITIONS:
                r = run_seed(config, db, s, uh, da, base=base)     # base+s -> mondes appariés entre conditions
        ...
        for name, _, _ in CONDITIONS:
            m = np.mean(agg[name]["mammoth"]); le = np.mean(agg[name]["leurre"]); su = np.mean(agg[name]["survivors"])
        ...
        h.save({name: {k: float(np.mean(v)) for k, v in agg[name].items()} for name in agg})
