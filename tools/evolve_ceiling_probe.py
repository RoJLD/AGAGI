...
def run_evolution(target, k_eras, num_agents, max_ticks, shared_db,
    ...
    if int(k_eras) <= 0 or int(num_agents) <= 0 or int(max_ticks) <= 0:
        raise ValueError(
            f"run_evolution : argument degenere (k_eras={k_eras}, num_agents={num_agents}, max_ticks={max_ticks}) -- aucune mesure possible ; "
