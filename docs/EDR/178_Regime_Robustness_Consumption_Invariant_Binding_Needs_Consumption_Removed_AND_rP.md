...
> `chantier/factorial-regime-sweep` (tag `keep/edr-177-178-factorial-regime-sweep`), porté dans HEAD par
> FUSION 3-voies — aucun conflit, gardes de HEAD conservées, cf. note d'EDR-177 — texte et chiffres ci-dessus
> INCHANGÉS. Le driver `tools/factorial_regime_sweep.py` ET le banc qu'il pilote (`compare_factorial` +
> leviers F1/F2/F4) sont dans HEAD : les tables ci-dessus sont RE-EXÉCUTABLES
> (`python tools/factorial_regime_sweep.py`). Au portage : `_factorial_effects` n'a qu'UNE définition, celle
> du banc, que le driver importe (corrigée porte 14 : un pool vide rend `nan`, pas `0.0`) ; `run_sweep` est
> un ORCHESTRATEUR qui accepte l'injection (`compare_fn`, `effects_fn`) et porte une garde d'arguments en
> tête ; le verdict de cellule-0 sous garde de puissance n ≥ 12 sort du bloc `__main__` vers une fonction
> PURE, `_cell0_verdict`. La couche PURE du driver (appariement régime → cellules, pivot, verdict) et
> l'orchestration du banc (garde en tête, 16 cellules distinctes, densité mappée sur `prey_count`, flags
> transmis à `run_arm` par seed apparié, no-op EXACT sous bruit, dose retrouvée en forme close du plan 2^4)
> sont calibrées par injection à dose connue dans `tests/sandbox/test_edr177_178_calibration.py` — 28 cas,
> AUCUN monde. Précision de lecture : le « NEUTRE (PLAT) » du régime rare est rendu à K=12, donc au-dessus
> du plancher — `_cell0_verdict` marque désormais NON-CONCLUANT tout verdict, nul compris, rendu SOUS ce
> plancher.
