...
> ✅ **RE-MESURÉ À BANDE RNG APPARIÉE, 2026-09-24 — règle scellée `S2-002-PAIRED-R1` ([[EDR-S2-002-PAIRED-R1]], `results/s2_002_paired_r1.json`) : lecture `CARTE_MODIFIEE`.** Le plancher de bruit de ±6-8 % dont parle le bandeau ci-dessus tombe à **ZERO** : le no-op apparié vaut **1,0000 EXACT sur 15/15 cellules** (5 clés de monde × 3 seeds, dont 2 seeds NEUFS), contre 1,058 / 0,922 à bande nue et 1,012 en P2.41 b. À cette résolution, deux lectures BOUGENT et deux seulement : **soup** (`within` médian 1,073) et **agricultural** (0,969) passent de `PERCEPTION_DECOY` à **`INCONCLUSIVE_DEGENERATE`** — non par changement de signe mais par le **PLANCHER** : leur bras INTACT est au niveau ou sous le plancher no-perception du monde (soup 32,00 / 29,25 / 28,50 contre plancher 32,00 ; agricultural 28,00 / 23,25 / 22,75 contre 25,25), et un ratio entre deux bras au plancher ne se lit pas. `stoneage`, `industrial` et `famine` restent `PERCEPTION_DECOY`, désormais à bruit nul. Le `between_ratio` des mêmes cellules vaut 3,57 à 5,42 quand le `within` vaut ~1,00 : le faux positif between-subject est reproduit sur 15 cellules sans bruit. Aucun verdict de ce record n'est rétracté.
