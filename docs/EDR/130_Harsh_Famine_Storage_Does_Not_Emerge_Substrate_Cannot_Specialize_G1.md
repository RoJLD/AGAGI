...
EDR-129 (transfert zéro-shot POSITIF) a laissé un **frontier explicite** : le transfert était « parfait »
parce qu'il n'existe qu'une compétence — la survie world-générale — et **aucune compétence
world-spécifique** (craft, stockage) n'émerge pour tester une vraie généralisation. EDR-126 avait montré
...
(`world_famine.py`, non-breaking, pattern HOF_PATH/MAX_ERAS d'EDR-126).

**Calibration** (`tools/famine_harshness_probe.py`, TDD 3/3) : sur le champion EDR-126, sweep `cyc_fam`
...
d'EDR-126, vrai seulement à famine courte).
...
appariée** : les 3 champions durs vs les 3 champions doux (EDR-126), tous mesurés dans le **même régime
...
| **DOUCE** (EDR-126) | 66.0 | 66.0 | 139.0 | **7.7** | **+0.0** |
...
- **Réponse au frontier d'EDR-129** : quand on FORCE une compétence world-spécifique (stockage exigé,
  oracle 2× buffer), elle **n'émerge pas** ; le substrat **dégrade au lieu de spécialiser**. Le transfert
  « noyau-partagé-seulement » d'EDR-129 n'est donc PAS un artefact des mondes choisis — c'est la **limite
...
3. **n=3 seeds** (smoke-confirmatoire, cohérent EDR-126) ; robuste sur le pattern (3/3 réserve≈douce, 3/3
...
4. **Isolation HoF** vérifiée (md5 stoneage global inchangé) ; déterminisme résiduel `main_biosphere`
   (memory_retriever non clear entre ères, comme EDR-126).
