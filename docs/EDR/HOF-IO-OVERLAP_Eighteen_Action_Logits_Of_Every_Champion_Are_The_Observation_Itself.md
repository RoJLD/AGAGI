...
2. **La lecture d'[[EDR-EVO-004]] doit être reprise.** Ce record conclut « saillance action/canal au
   ...
   ⚠️ **REPRIS le 2026-09-15, et l'attribution était FAUSSE** : les sujets d'[[EDR-EVO-004]] sont **quatre champions évolués in-world** (`evolve_inworld`, 64-dim natifs, sans chevauchement — 0/348 génomes persistés au balayage du 09-08), **pas le champion HoF**. Sa conclusion « plancher sur tous les canaux » n'est pas confondue par E24. Le champion HoF, lui, ne pouvait pas y être mesuré tel quel (monde à 59 entrées, `recurrent_forward` ne complète pas). Mesure du 2026-09-15 (`results/evo004_relecture_e24_champion.json`, sonde de saillance en mode DÉCISION, 8 agents × 40 ticks, stoneage, obs complétée à 64 comme le fait `MambaBatchModel.forward`) : canaux partagés 46-53 (`in_mem[0..4]`, `in_confort`, `is_night`, `fire_nearby`) bascule médiane **0,51** (`in_mem0` 0,90, `in_confort` 0,86) ; les 56 autres canaux médiane **0,003** — mais `lidar_n` 0,83, `adj_energy` 0,74, `ax` 0,62, `terrain_4`/`in_surprise`/`age` 0,50-0,67 sont LUS, à travers des poids ; `argmax(logits[:8]) == argmax(obs[46:54])` sur **71 %** des décisions ; l'identité `logits[:18] = obs[46:64]` n'est qu'approximative avec l'état récurrent en marche (écart médian 0,43). n = 1 seed, exploratoire. Ce que ça change : le champion HoF **lit** l'observation, par identité ET par poids — `PERCEPTION_DECOY` (S2-002/003) signifie « lire ne paie pas la survie », pas « ne lit pas » (cf. P2.59 : aucune lecture câblée n'est net-positive sur stoneage).
