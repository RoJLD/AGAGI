...
- frontiere_courante: EDR-NAV-001 → mur navigation = READOUT_GAP (H décode ~0.81, tête d'action l'ignore ; encodeur OK) ; EDR-NAV-002 → détresse énergétique aussi dans H (0.89-0.91) mais readout énergie non isolable (endogène) ; EDR-NAV-003 → le readout NAV EST RL-récupérable (recovery +0.92, offline sur H figé) → gap in-world = signal, pas trainabilité ; EDR-NAV-004 → frontière de récupération : sparsité (1%→+0.91) ET bruit (90%→+0.95) QUASI-GRATUITS, misattribution BIAISÉE FATALE (β=0.6→−0.93) = le levier T1 est l'ALIGNEMENT per-pas, pas la densité ; EDR-NAV-005 → le mur in-world du binding d'EDR-172 (throw-gate) = le BIAIS (−0.5 sur throw non-payant), PAS la rareté : non-biaisé récupère à 3% succès (+0.97), biaisé s'effondre sous p_success<1/3 (−0.93, reproduit gap_ON<0) → correctif actionnable T3 = récompense NON-BIAISÉE (raffine la couche 2 d'EDR-172 ; couche 1 survie séparée). Reste = M2 in-world (session torch, touche backend_torch)
...
- fichiers_possedes: tools/tom_probe.py, tools/anticipation_bench.py, tools/disjoint_heads_ab.py, tools/disjoint_heads_v4.py
...
- frontiere_courante: EDR-COG-001 → lr-par-tête (readouts seuls) NE recouvre PAS (−0.16, LR_INSUFFICIENT ; échelle 0.79/moments 0.73 reproduits) → l'équilibrage de crédit vit dans le TRONC partagé, pas les têtes → « crédit pas archi » se précise en « crédit sur le tronc » ; angle proxy disjoint CLOS. Port prod = GradNorm-lite échelle (153), PAS lr-par-tête. + volet causal ToM NON TESTÉ (incitation ToM→life_score, différé migration torch — EDR 150/151)
