...
- frontiere_courante: EDR-NAV-001 → mur navigation = READOUT_GAP (H décode ~0.81, tête d'action l'ignore ; encodeur OK) ; EDR-NAV-002 → détresse énergétique aussi dans H (0.89-0.91) mais readout énergie non isolable (endogène) ; EDR-NAV-003 → le readout NAV EST RL-récupérable (recovery +0.92, offline sur H figé) → gap in-world = signal, pas trainabilité ; EDR-NAV-004 → frontière de récupération : sparsité (1% des pas → +0.91) ET bruit (90% → +0.95) QUASI-GRATUITS, mais misattribution BIAISÉE FATALE (β=0.6 → −0.93, apprend le faux) = BIAS_IS_FATAL → le levier T1 est l'ALIGNEMENT per-pas, PAS la densité (aux supervisé oracle sûr ; shaping OK si per-pas aligné, JAMAIS trajectoire-agrégé=biais). Reste = M2 in-world (session torch, touche backend_torch)
