...
`FADE_INEFFECTIVE` qui priment sur les chiffres aval). Le cliquet bloque tout NOUVEAU non calibré : **234 détectés** <!-- count:instruments_detectes=234 --> / **225 calibrés** <!-- count:instruments_calibres=225 --> / **3 dettes** <!-- count:instruments_non_calibres=3 --> gelées et NOMMÉES (P2.62, 2026-09-15 : les apprenants legacy `compute_policy_gradient`, `learn` hors backend torch, `learn_episode_bptt` — la baseline n'est plus vide, et elle est déclenchante pour le hook). **Les runs proxy de niveau 1 sont DÉBLOQUÉS.**
