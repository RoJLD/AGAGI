...
- REWORK LIVRE (commit ae3de29, 9/9 tests, SPEC OK verbatim + QUALITE Approved).
- RUN REEL (seeds 0-7 w300/m600, 2 passes BYTE-IDENTIQUES) : VERDICT LATENT_BILINEAR. HIDDEN learned-lin=0.358 (fidele!) vs bilin=0.021 (~17x mieux). ENCODING_ARTIFACT REFUTE (opus avait predit ARTIFACT -> miss ; le latent cache EST anticipable). Diagnostic post-hoc action-conditionnement : HIDDEN per-action 0.021 vs agnostique 0.072 (x3.4 -> conditionnement reel) mais agnostique deja fidele (recurrence quasi-lineaire-en-H). -> la FORME de g (bilineaire) est un levier G4 reel ; caveats : fit offline=oracle, fidelite != comportement, recurrence quasi-lineaire. gb_pass1/2.txt.
- Reste : commit EDR 193 + memoire + PR.
